 ...
 */
export async function generate3DScatterChart(
  username: string,
  platform: string,
  datasets: Dataset3D[],
  totalCommits: number,
  ...
  console.log(`[3D] Rendering ${datasets.length} isometric 3D scatter dataset(s)...`);
  ...
  datasets.forEach((dataset, datasetIndex) => {
    ...
    // Draw scatter points as spheres
    for (let i = 0; i < points.length; i++) {
      ...
      // Draw sphere with radius based on commit value
      const radius = Math.max(5, Math.min(points[i] * 0.3, 15));
